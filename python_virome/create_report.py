...
tsne = open(typespecific + design + "_" + tissue + "_numeric_viral_exposureds.csv", "w")
outf = open(typespecific + design + "_" + tissue + "_viral_exposured_agedays.csv", "w")
adj_paired = open(typespecific + design + "_" + tissue + "_viral_exposured_agedays_adj_paired.csv", "w")
adj_sample = open(typespecific + design + "_" + tissue + "_viral_exposured_agedays_adj_sample.csv", "w")
...
for r in srows:
	...
	virus_corrected = [str(x/1) for x in [_evprior, _coxaprior, _coxbprior, _coxb3prior, _coxb3nprior, _cmvprior, _noroprior, _crassprior, _parvoprior, _rotaprior, _rhinoprior]]	
	...
	numfiles = 1
	if (outcome == "1"):
		if (case_f.get(case_ind) != None):
			numfiles = case_f[case_ind]
	if (outcome == "0"):
		if (control_f.get(case_ind) != None):
